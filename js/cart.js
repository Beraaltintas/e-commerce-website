  ...
  cart.forEach((item) => {
    result += `                  
        <tr class="cart-item">
         ...
         <td class="product-quantitiy">${item.quantity}</td>
         <td class="product-subtotal">$${(item.price.newPrice * item.quantity).toFixed(2)}</td>
        </tr>`;
  });
  cartWrapper.innerHTML = result; /* html e yazar bu kod*/
  removeCartItem();
}
displayCartProduct();
...
function removeCartItem() {
    ...
    btnDeleteCart.forEach((button)=> {
      button.addEventListener("click", function(e){
          ...
          cartItems.innerHTML =cart.length;//sayıyı günceller
          saveCartValues();
      })
...
function saveCartValues(){
    ...
    let itemsTotal =0;
    cart.length > 0 && cart.map((item)=> (itemsTotal += item.price.newPrice * item.quantity)); //map arrayler içinde döngü sağlar
